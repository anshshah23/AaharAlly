...
def process_data():
    ...
    age_food_group_counts = df.groupby(['age_group', 'category']).size().unstack(fill_value=0)
    ...
    result['Dish with Age Range'] = result['Most Ordered Dish'] + ' (' + result.index.astype(str) + ')'
